...
export class PlayerMoves {
  ...
  undoMove(): void {
    if(this.redoMoves.length > 0) {
      this.redoMoves[0].undo();
      this.undoMoves.push(this.redoMoves[0]);
      this.redoMoves.splice(0, 1);
    }
    else if(this.pastMoves.length > 0) {
      this.pastMoves[0].undo();
      this.undoMoves.push(this.pastMoves[0]);
      this.pastMoves.splice(0, 1);
    }
    if(this.redoMoves.length > 0) this.redoMoves[0].state = "currentMove";
    else if(this.pastMoves.length > 0) this.pastMoves[0].state = "currentMove";
  }

  redoMove(): void {
    if(this.redoMoves.length > 0) this.redoMoves[0].state = "past";
    else if(this.pastMoves.length > 0) this.pastMoves[0].state = "past";
